...
public class App {
    ...
    public static void main(String[] args) {
        ...
        // Kick off workflow
        //   load subject IDs from link file
        if (getSetting(properties, "useLinkFile").equals("true")) {
            File linkFile = new File( getSetting(properties, "linkFile") );
            rdfStatements.getSubjects(linkFile);
            esBulkDump.close();
            rdfStatements.close();
        } 
        //   load subject IDs from triplestore
        else {
            for (String type : getSetting(properties, "types").split("#")) {
                rdfStatements.getSubjects(type);
            }
            esBulkDump.close();
            rdfStatements.close();
        }
